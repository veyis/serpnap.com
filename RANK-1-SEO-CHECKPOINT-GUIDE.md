...
_If Google can't crawl and index you, nothing else matters._
...
- [x] **P1** `robots.txt` points to sitemap; allows all important crawlers (Googlebot, Bingbot, AI bots).
...
- [x] **P0** HTTPS sitewide — valid SSL certificate; HTTP 301-redirects to HTTPS.
- [x] **P1** HSTS header: `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`.
- [x] **P1** No mixed content — zero HTTP resources loaded on HTTPS pages.
- [x] **P1** Security headers present: `X-Content-Type-Options: nosniff`, `X-Frame-Options` (or CSP `frame-ancestors`), `Referrer-Policy`.
...
- [x] **P1** `<link rel="canonical" href="...">` on every page pointing to preferred version.
...
_Failing CWV correlates with ~24% lower organic CTR. Google uses 75th percentile real-user data._
...
- [x] **P1** Show dates — publish date and "last updated" date visible on content pages.
...
_Trust is the center of E-E-A-T — without trust, the other signals carry less weight. Not a direct ranking factor, but the quality signals Google evaluates map directly to E-E-A-T._
...
- [x] **P2** Expert bylines on all content — real names, real bios, real photos.

### 5.3 Authoritativeness

- [x] **P1** Clear "About Us" page with company history, team, credentials.
...
- [x] **P1** `FAQPage` on pages with 2+ question/answer pairs.
...
- [x] **P2** `SpeakableSpecification` on key pages for voice search eligibility.
...
_For service area businesses: GBP + website + citations must tell one consistent story. Google evaluates you as a unified entity._
...
_Quality over quantity. One editorial link from a relevant, authoritative site > 100 directory links._
...
_Accessibility overlaps heavily with SEO — headings, alt text, semantic HTML all serve both._
...
_Google tracks user behavior signals — pogo-sticking (returning to search after clicking) indicates poor result quality._
...
_Freshness is now ~6% of Google's algorithm. Pages updated within 12 months gain avg. +4.6 positions._

- [ ] **P1** Top 20 landing pages reviewed and updated at least annually.
- [ ] **P1** Blog publishing cadence: minimum 2-4 posts/month.
- [x] **P1** "Last updated" dates visible on service and guide pages.
...
_47% of AI Overview citations come from pages ranking below position #5. AI systems reward content quality and extractability over domain authority._
...
- [x] **P1** `robots.txt` allows AI crawlers: GPTBot, ChatGPT-User, Google-Extended, ClaudeBot, PerplexityBot, CCBot.
- [x] **P1** `llms.txt` file at site root — structured summary for AI models.
...
_Sites without ongoing monitoring see ~12% average quarterly traffic decline when issues accumulate._
...
_Created: March 2026. Combines WEBSITE-CHECKPOINT-GUIDE-ULTIMATE.md + GOOGLE-SEO-CHECKPOINT-GUIDE.md + 2026 research. Review and update quarterly._
